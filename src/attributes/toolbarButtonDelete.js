...
wp.hooks.addFilter(
    ...
    'custom-attributes/set-toolbar-button-attribute-delete',
...
wp.hooks.addFilter(
    'editor.BlockEdit',
    'custom-attributes/with-toolbar-button-delete',
    withToolbarButton
);
...
wp.hooks.addFilter(
    ...
    'custom-attributes/with-toolbar-button-prop-delete',
    withToolbarButtonProp
);
...
const saveToolbarButtonAttribute = ( extraProps, blockType, attributes ) => {
    ...
    if ( disableToolbarButtonOnBlocks.includes( blockType.name ) ) {
        const { paragraphAttribute } = attributes;
        if ( paragraphAttribute && 'custom' === paragraphAttribute ) {
            extraProps.className = classnames( extraProps.className, 'has-custom-attributeZ' )
        }
    }    
...
wp.hooks.addFilter(
    ...
    'visual-editor-toolbar/delete-button',
