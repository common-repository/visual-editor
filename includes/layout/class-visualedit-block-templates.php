...
if ( ! class_exists( 'VISUALEDIT_Block_Templates' ) ) :

	/**
	 * Admin
	 */
	class VISUALEDIT_Block_Templates {

		/**
		 * Instance
		 *
		 * @since 1.0.2
		 * @var (Object) VISUALEDIT_Block_Templates
		 */
		...
		private function __construct() {
			require_once VISUAL_EDITOR_BLOCKS_DIR . '/includes/layout/functions.php';
			require_once VISUAL_EDITOR_BLOCKS_DIR . '/includes/layout/class-visualedit-block-templates-image-importer.php';
			include_once ABSPATH . 'wp-admin/includes/image.php';

			add_action( 'wp_ajax_visualedit_block_templates_import_block', array( $this, 'import_block' ) );
		}
		...
		public function get_content( $content = '' ) {
			...
			foreach ( $all_links as $key => $link ) {
				if ( visualedit_block_templates_is_valid_image( $link ) ) {
			...
			if ( ! empty( $image_links ) ) {
				foreach ( $image_links as $key => $image_url ) {
					...
					$downloaded_image = VISUALEDIT_Block_Templates_Image_Importer::get_instance()->import( $image );
	 ...
	 */
	VISUALEDIT_Block_Templates::get_instance();
