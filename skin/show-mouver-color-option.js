function visualeditorMouverColorOption(element, components, data, compose) {
    ...
    const dispatchMeta = function (dispatch, props) {
        return {
            setMetaValue: function (metaValue) {
                ...
                let visualeditorMouverColorsMeta = {};
                visualeditorMouverColorsMeta[props.metaKey] = metaValue;
                const completeDispatch = dispatch('core').saveUser({
                    id: user.id,
                    meta: { ...user.meta, ...visualeditorMouverColorsMeta },
                });

                completeDispatch.then(() => {
                    let root = document.documentElement;
                    root.style.setProperty('--mouver-color', metaValue);
