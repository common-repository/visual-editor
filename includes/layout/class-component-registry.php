...
final class Component_Registry {
	...
	public static function add( array $data ) {
		...
		if ( empty( $data['type'] ) || ! in_array( $data['type'], self::$supported_component_types, true ) ) {
			throw new InvalidArgumentException( esc_html__( 'You must supply a valid component type.', 'visual-editor-blocks' ) );
		}

		if ( empty( $data ) ) {
			throw new InvalidArgumentException( __( 'You must supply valid layout data to register a layout.', 'visual-editor-blocks' ) );
		}
		...
		foreach ( self::$required_data_keys as $required_key ) {
			if ( ! array_key_exists( $required_key, $data ) || empty( $data[ $required_key ] ) ) {
				...
				throw new InvalidArgumentException( sprintf( esc_html__( 'You must supply a %s to register a layout.', 'visual-editor-blocks' ), $required_key ) );
			}
		}
		...
		switch ( $data['type'] ) {
			case 'layout':
				if ( ! empty( self::$layouts[ $data['key'] ] ) ) {
					...
					throw new InvalidArgumentException( sprintf( esc_html__( 'The %s layout is already registered.', 'visual-editor-blocks' ), $data['key'] ) );
				}
			...
			case 'section':
				if ( ! empty( self::$sections[ $data['key'] ] ) ) {
					/* translators: %s: The component's unique key. */
					throw new InvalidArgumentException( sprintf( esc_html__( 'The %s section is already registered.', 'visual-editor-blocks' ), $data['key'] ) );
				}
				self::$sections[ $data['key'] ] = $data;
				break;

			default:
				/* translators: %s: This function's name. Will always be VISUALEDIT\Blocks\Layouts\Component_Registry::add(). */
				throw new InvalidArgumentException( sprintf( esc_html__( 'You must supply a valid component type in %s.', 'visual-editor-blocks' ), __METHOD__ ) );
		}
	}
	...
	public static function remove( $type, $key ) {
		...
		if ( empty( $type ) || ! in_array( $type, self::$supported_component_types, true ) ) {
			/* translators: %s: This function's name. Will always be VISUALEDIT\Blocks\Layouts\Component_Registry::remove(). */
			throw new InvalidArgumentException( sprintf( esc_html__( 'You must supply a valid component type in %s.', 'visual-editor-blocks' ), __METHOD__ ) );
		}

		if ( empty( $key ) ) {
			/* translators: %s: This function's name. Will always be VISUALEDIT\Blocks\Layouts\Component_Registry::remove(). */
			throw new InvalidArgumentException( sprintf( esc_html__( 'You must supply a valid component key in %s.', 'visual-editor-blocks' ), __METHOD__ ) );
		}
		...
		switch ( $type ) {
			case 'layout':
				if ( empty( self::$layouts[ $key ] ) ) {
					...
					throw new Exception( sprintf( esc_html__( 'The %s layout is not registered.', 'visual-editor-blocks' ), $key ) );
				}
			...
			case 'section':
				if ( empty( self::$sections[ $key ] ) ) {
					/* translators: The requested components unique key. */
					throw new Exception( sprintf( esc_html__( 'The %s section is not registered.', 'visual-editor-blocks' ), $key ) );
				}
	...
	public static function get( $type, $key ) {
		...
		if ( empty( $type ) || ! in_array( $type, self::$supported_component_types, true ) ) {
			/* translators: This function name. Will always be VISUALEDIT\Blocks\Layouts\Component_Registry::get(). */
			throw new Exception( sprintf( esc_html__( 'You must supply a component type in %s.', 'visual-editor-blocks' ), __METHOD__ ) );
		}
