...
use VISUALEDIT\Blocks\Layouts\Component_Registry;
 ...
 */
function visualedit_blocks_register_layout_component( array $data ) {
 ...
 */
function visualedit_blocks_unregister_layout_component( $type, $key ) {
	$registry = Component_Registry::instance();
	try {
 ...
 */
function visualedit_blocks_get_layout_component( $type, $key ) {

	if ( empty( $type ) ) {
		return new WP_Error( esc_html__( 'You must supply a type to retrieve a layout component.', 'visual-editor-blocks' ) );
	}

	if ( empty( $key ) ) {
		return new WP_Error( esc_html__( 'You must supply a key to retrieve a layout component.', 'visual-editor-blocks' ) );
	}
 ...
 */
function visualedit_blocks_get_layouts() {
	$registry = Component_Registry::instance();
 ...
 */
function visualedit_blocks_get_sections() {
	$registry = Component_Registry::instance();
