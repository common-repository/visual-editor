 ...
 *
 * @package VISUALEDIT Block Templates
 * @since 1.0.2
 */

if ( ! class_exists( 'VISUALEDIT_Block_Templates_Image_Importer' ) ) :

	/**
	 * VISUALEDIT Templates Image Importer
	 *
	 * @since 1.3.4
	 */
	class VISUALEDIT_Block_Templates_Image_Importer {
			...
			$post_id = $wpdb->get_var(
				$wpdb->prepare(
					'SELECT `post_id` FROM `' . $wpdb->postmeta . '`
						WHERE `meta_key` = \'_visualedit_templates_image_hash\'
							AND `meta_value` = %s
					;',
		...
		public function import( $attachment ) {
			...
			update_post_meta( $post_id, '_visualedit_templates_image_hash', $this->get_hash_image( $attachment['url'] ) );
		...
		public function is_image_url( $url = '' ) {
			...
			if ( visualedit_block_templates_is_valid_image( $url ) ) {
				return true;
			}
	 ...
	 */
	VISUALEDIT_Block_Templates_Image_Importer::get_instance();
