...
require_once VISUAL_EDITOR_BLOCKS_DIR . '/includes/layout/class-visualedit-block-templates.php';

const VISUALEDIT_API_NAMESPACE = 'visualeditblocks/v1';
...
function register_layout_endpoints() {
	 ...
	 */
	register_rest_route(
		VISUALEDIT_API_NAMESPACE,
		FAVORITE_LAYOUTS_ROUTE,
		array(
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				return new WP_REST_Response( (array) get_user_meta( get_current_user_id(), 'visualedit_blocks_favorite_layouts', true ) );
			},
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function ( \WP_REST_Request $request ) {
				...
				$layouts            = visualedit_blocks_get_layouts();
				$sections           = visualedit_blocks_get_sections();
				$additional_layouts = apply_filters( 'visualedit_blocks_additional_layout_components', array() );
				 ...
				 */
				$allowed_layouts = (array) apply_filters( 'visualedit_blocks_allowed_layout_components', array_keys( $all_layouts ) );
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				return new WP_REST_Response( (array) visualedit_blocks_get_layouts() );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function ( $request ) {
				...
				return new WP_REST_Response( esc_html__( 'Layout not found.', 'visual-editor-blocks' ) );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function ( $request ) {
				...
				$new       = sanitize_key( $body['visualedit_blocks_favorite_key'] );
				$favorites = (array) get_user_meta( get_current_user_id(), 'visualedit_blocks_favorite_layouts', true );
				...
				update_user_meta( get_current_user_id(), 'visualedit_blocks_favorite_layouts', array_values( $favorites ) );
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function ( $request ) {
				...
				$body      = json_decode( $request->get_body(), true );
				$delete_id = sanitize_key( $body['visualedit_blocks_favorite_key'] );
				$favorites = (array) get_user_meta( get_current_user_id(), 'visualedit_blocks_favorite_layouts', true );
	...
	register_rest_route(
		...
		array(
			'methods'             => WP_REST_Server::READABLE,
			'callback'            => function () {
				return new WP_REST_Response( (array) visualedit_blocks_get_sections() );
			},
			'permission_callback' => function () {
				return current_user_can( 'edit_posts' );
			},
	...
	register_rest_route(
		...
		array(
			...
			'callback'            => function ( $request ) {
				...
				$modified_content = do_action( 'wp_ajax_visualedit_block_templates_import_block', $content );
